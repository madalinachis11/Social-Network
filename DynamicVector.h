...
public:
	...
	DynamicVector* operator+(const T& newElement);
	...
	void UpdateElement(T& elem, const T& new_elem);
...
inline void DynamicVector<T>::RemoveElement(size_t position)
{
	...
	for (size_t i = position; i < this->currentSize; ++i)
...
template<typename T>
inline void DynamicVector<T>::UpdateElement(T& elem, const T& new_elem)
{
	int ok = 0;
	if (find(elem) == 1)
		ok = 1;
	replace(elements.begin(), elements.end(), elem, new_elem);
	return ok;
}
